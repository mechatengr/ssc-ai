...
function createSession() {
  const s = { id: uid(), title: "New Conversation", pinned: false, createdAt: Date.now(), updatedAt: Date.now(), messages: [] };
...
$("#persona-trigger").addEventListener("click", () =>
  openPopover($("#persona-trigger"), PERSONAS, {
    ...
    onSelect: (id) => { state.persona = id; localStorage.setItem("ssc_persona", id); refreshTopbarLabels(); },
  })
);
$("#model-trigger").addEventListener("click", () =>
  openPopover($("#model-trigger"), MODELS, {
    selectedId: state.model,
    onSelect: (id) => { state.model = id; localStorage.setItem("ssc_model", id); refreshTopbarLabels(); },
    width: "260px",
  })
);
$("#websearch-toggle").addEventListener("click", () => { state.webSearch = !state.webSearch; localStorage.setItem("ssc_websearch", state.webSearch ? "1" : "0"); refreshTopbarLabels(); });
$("#deepthink-toggle").addEventListener("click", () => { state.deepThink = !state.deepThink; localStorage.setItem("ssc_deepthink", state.deepThink ? "1" : "0"); refreshTopbarLabels(); });
$("#mastermind-toggle").addEventListener("click", () => { state.mastermind = !state.mastermind; localStorage.setItem("ssc_mastermind", state.mastermind ? "1" : "0"); refreshTopbarLabels(); });
...
function handleSessionAction(action, session) {
  if (action === "rename") {
    ...
    if (newTitle && newTitle.trim()) { session.title = newTitle.trim(); persistSessions(); renderSessionList(); if (session.id === state.currentSessionId) $("#topbar-title").textContent = session.title; }
  } else if (action === "pin") {
...
function forkConversation(m) {
  const s = getCurrentSession();
  const idx = s.messages.findIndex((x) => x.id === m.id);
  const forked = { id: uid(), title: s.title + " (fork)", pinned: false, createdAt: Date.now(), updatedAt: Date.now(), messages: JSON.parse(JSON.stringify(s.messages.slice(0, idx + 1))) };
...
function sendMessage() {
  ...
  if (s.messages.length === 1) generateSmartTitle(s);
      ...
      while (true) {
        ...
        for (const evt of events) {
          ...
          if (eventType === "chunk") {
            ...
            if (!gotFirstChunk) bubble.innerHTML = `<div class="typing-dots" title="${data.stage}"><span></span><span></span><span></span></div>`;
...
async function generateSmartTitle(session) {
  try {
    ...
    if (title) { session.title = title; persistSessions(); renderSessionList(); $("#topbar-title").textContent = title; }
...
function renderSettingsBody() {
  ...
  if (activeSettingsTab === "Language") {
    const sel = el("select", { onchange: (e) => { state.language = e.target.value; localStorage.setItem("ssc_language", e.target.value); } });
