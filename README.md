...
├── render.yaml                Render Blueprint — deploys the backend with
│                             the correct settings automatically (see below)
...
2. Deploy `backend/` to a free host (Render.com) — **use the "Blueprint"
   deploy option and point it at this repo's `render.yaml`** rather than
   manually creating a Web Service. This automatically sets the correct
   Root Directory, build/start commands, and Node version, which avoids the
   most common deploy failure (Render building from the repo root instead
   of `backend/`, which has no `start` script there).
